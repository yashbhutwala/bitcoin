...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # This applies to both wallet "w" (restored from backup) and wallet "w1"
        # (watch-only created at snapshot height).
        assert_raises_rpc_error(-4, "Wallet loading failed. Error loading wallet. Wallet requires blocks to be downloaded", n1.loadwallet, "w")
        assert_raises_rpc_error(-4, "Wallet loading failed. Error loading wallet. Wallet requires blocks to be downloaded", n1.loadwallet, wallet_name)

        # Verify backup from before snapshot height still can't be restored
        assert_raises_rpc_error(-4, "Wallet loading failed. Error loading wallet. Wallet requires blocks to be downloaded", n1.restorewallet, "w2_test", "backup_w2.dat")
